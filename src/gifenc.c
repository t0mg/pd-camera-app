...
static void
write_num(ge_GIF *gif, int n)
{
  /* helper to write a little-endian 16-bit number portably */
  gif->pd->file->write(gif->fd, (uint8_t []) {(n) & 0xFF, (n) >> 8}, 2);
}
...
)
{
    ...
    // char* filename = (char *) malloc( strlen(path) + 5 ); 
    // strcpy(filename, path);
    // strcat(filename, ".gif");
    gif->fd = gif->pd->file->open(filename, kFileWrite);
    // free(filename);
// #ifdef _WIN32
...
// #endif
    gif->pd->file->write(gif->fd, "GIF89a", 6);
    write_num(gif, width);
    write_num(gif, height);
    ...
    gif->pd->file->write(gif->fd, (uint8_t []) {0xF0 | (depth-1), (uint8_t) bgindex, 0x00}, 3);
    if (custom_gct) {
        gif->pd->file->write(gif->fd, palette, 3 << depth);
    } else if (depth <= 4) {
...
{
    gif->pd->file->write(gif->fd, (uint8_t []) {'!', 0xFF, 0x0B}, 3);
    gif->pd->file->write(gif->fd, "NETSCAPE2.0", 11);
    gif->pd->file->write(gif->fd, (uint8_t []) {0x03, 0x01}, 2);
    write_num(gif, loop);
    gif->pd->file->write(gif->fd, "\0", 1);
}
...
put_key(ge_GIF *gif, uint16_t key, int key_size)
{
    ...
    while (bits_to_write >= 8) {
        ...
        if (byte_offset == 0xFF) {
            gif->pd->file->write(gif->fd, "\xFF", 1);
            gif->pd->file->write(gif->fd, gif->buffer, 0xFF);
            byte_offset = 0;
        }
        gif->partial >>= 8;
        bits_to_write -= 8;
...
end_key(ge_GIF *gif)
{
    ...
    if (gif->offset % 8)
        gif->buffer[byte_offset++] = gif->partial & 0xFF;
    if (byte_offset) {
        gif->pd->file->write(gif->fd, (uint8_t []) {byte_offset}, 1);
        gif->pd->file->write(gif->fd, gif->buffer, byte_offset);
    }
    gif->pd->file->write(gif->fd, "\0", 1);
...
put_image(ge_GIF *gif, uint16_t w, uint16_t h, uint16_t x, uint16_t y)
{
    ...
    gif->pd->file->write(gif->fd, ",", 1);
    write_num(gif, x);
    write_num(gif, y);
    write_num(gif, w);
    write_num(gif, h);
    gif->pd->file->write(gif->fd, (uint8_t []) {0x00, gif->depth}, 2);
...
add_graphics_control_extension(ge_GIF *gif, uint16_t d)
{
    ...
    gif->pd->file->write(gif->fd, (uint8_t []) {'!', 0xF9, 0x04, flags}, 4);
    write_num(gif, d);
    gif->pd->file->write(gif->fd, (uint8_t []) {(uint8_t) gif->bgindex, 0x00}, 2);
}
